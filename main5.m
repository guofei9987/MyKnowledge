%自编BP
%因为最后一层是线性不好使，所以参考案例，把最后一层改成sigmoid
    ...
    while (c<=samplenum)%对于每个样本输入，计算输出，进行一次BP训练
        ...
        for layer_ind=2:numlayers
            for j=1:num_per_layer(layer_ind)
                ...
                v{layer_ind}(j)= 1/(1+exp(-u{layer_ind}(j)));%中间层与输出层全是sigmoid
            end
        end
        %end
        ...
        %最后一层的梯度
            ...
            for j=1:num_per_layer(numlayers)
                delta_w{numlayers}(j)=(d(j)-v{numlayers}(j))*v{numlayers}(j)*(1-v{numlayers}(j));
            end
        ...
        layer_ind=2;
            for i=1:num_per_layer(layer_ind)
                ...
                for j=1:num_per_layer(layer_ind+1)
                    temp=temp+w{layer_ind+1}(i,j)*delta_w{layer_ind+1}(j);
