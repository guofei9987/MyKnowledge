...
while (count<=maxcount) %结束条件1迭代1000次
    ...
    while (c<=samplenum)%对于每个样本输入，计算输出，进行一次BP训练
        ...
        for k=1:outputNums %求每个输出节点的值!!这一块不对，最终层应当是线性的
            net=0.0;
            for j=1:hideNums
                net=net+y(j)*w(j,k);
            end
        ...
        for j=1:hideNums  %调节到每个隐藏点到输出点的权重
            for k=1:outputNums
                deltw(j,k)=eta*yitao(k)*y(j); %权值w的调整量deltw(已乘学习率)
        ...
        for i=1:inputNums  %调节每个输入点到隐藏点的权重
            for j=1:hideNums
                deltv(i,j)=eta*yitay(j)*x(i); %同上deltw
